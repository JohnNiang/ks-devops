...
func (h *handler) handleQuery(request *restful.Request, response *restful.Response) {
	if data, err := h.queryTemplate(request); err != nil {
		...
		if err := response.WriteEntity(data); err != nil {
			klog.V(8).Infof("failed to write template list to response stream while querying templates, err = %v", err)
		}
	}
}
...
func (h *handler) handleCreate(request *restful.Request, response *restful.Response) {
	if template, err := h.createTemplate(request); err != nil {
		kapis.HandleError(request, response, err)
	} else {
		if err := response.WriteEntity(template); err != nil {
			klog.V(8).Infof("failed to write template to response stream while creating template, err = %v", err)
		}
	}
}

func (h *handler) createTemplate(request *restful.Request) (*v1alpha1.Template, error) {
	devopsName := request.PathParameter(DevopsPathParameter.Data().Name)
	// get template body
	template := &v1alpha1.Template{}
	if err := request.ReadEntity(template); err != nil {
		return nil, err
	}
	template.SetNamespace(devopsName)
	if err := h.genericClient.Create(context.Background(), template); err != nil {
		return nil, err
	}
	return template, nil
}
...
func (h *handler) handleGet(request *restful.Request, response *restful.Response) {
	if template, err := h.getTemplate(request); err != nil {
		...
		if err := response.WriteEntity(template); err != nil {
			klog.V(8).Infof("failed to write template to response stream while getting template, err = %v", err)
		}
	}
}
...
func (h *handler) handleRender(request *restful.Request, response *restful.Response) {
	if template, err := h.renderTemplate(request); err != nil {
		...
		if err := response.WriteEntity(template); err != nil {
			klog.V(8).Infof("failed to write template to response stream while rendering template, err = %v", err)
		}
	}
}
...
func (h *handler) renderTemplate(request *restful.Request) (*v1alpha1.Template, error) {
	...
	if err := request.ReadEntity(parameters); kapis.IgnoreEOF(err) != nil {
		return nil, err
	}
